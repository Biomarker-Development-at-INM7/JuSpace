...
opt_rank = get(handles.plot_rank,'Value');
Nperm = str2num(get(handles.nperm,'String'));
opt_plot = find([handles.plot_opt.Children.Value]==1);
...
%     set(gca,'XTickLabel',Rec_list);
    ...
    for j=1:length(h3)
        patch(get(h3(j),'XData'),get(h3(j),'YData'),flip_color(j,:),'EdgeColor','none');
    end
    hold on
...
for i = 1:length(filesPET)
    ...
    if opt_comp>=4
        ...
        if n_files>1
        plot(x_n_n(ind_plot==i),vv2(ind_plot==i),'o','MarkerSize',8,'MarkerFaceColor',bar_color(i,:),'MarkerEdgeColor','black','LineWidth',1);
        end
    end
end
...
%     n_color = 1./(n_draw+1);
    ...
    if ind_col==1
        hh = figure('Position',[400 0 n_rows.*500 n_rows.*500]);
        subplot(n_rows,size(data_PET,1),1);
    else

        hh = figure('Position',[400 0 1200 n_rows.*350]);
    ...
    if opt_plot == 2
        ...
        color_i = generate_colors_nice_my(size(data_PET,1));
    end
...
%         else
    if opt_rank == 1
        xlabel(['Rank (Data ' Rec_list{j} ')'],'FontSize',16);
    else
...
%                         end
                        if opt_plot == 2
                            ...
                            plot(x,y,'o','MarkerSize',9,'MarkerFaceColor',color_i(j,:),'MarkerEdgeColor','black');
                            colors_all = color_i;
                        end
...
%         end
        ...
        if opt_plot == 2
            ...
            for i = 1:length(h1)
               h1(i).Color = colors_all(j,:); 
            end
        end
            if size(data,1)==1 & opt_plot == 2
                [aa,ind_a] = sort(y);
    ...
    if size(data_PET,1)==1
        if opt_rank == 1
            ylabel('Rank Data Modality','FontSize',16);
        else
...
%         p4=get(h4,'position');
        ...
        h5=axes('position',[p3(1)-0.04 p3(2).*(1 + 1./n_rows) width height],'visible','off'); 
...
%         else
        if opt_rank == 1
            ylabel('Rank Data Modality','FontSize',20);
        else
...
% --- Executes on button press in plot_rank.
function plot_rank_Callback(hObject, eventdata, handles)
% hObject    handle to plot_rank (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
